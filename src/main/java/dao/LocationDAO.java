...
public class LocationDAO {
	...
	private static final String SQL_DELETE_LOCATION_WITH_ID = "DELETE FROM location WHERE id = ?";
	...
	public static boolean insert(Location location) {
		...
		Object values[] = { location.getStreetAddress(), location.getStreetNumber(), location.getPostalCode(),
				location.getCity(), location.getCountryId() };
		try {
	...
	public static Location getLocationIfExist(String streetAddress, int streetNumber, String postalCode, String city,
			int countryId) {
		...
		try {
			connection = connectionPool.checkOut();
			PreparedStatement pstmt = DAOUtil.prepareStatement(connection, SQL_SELECT_LOCATION_IF_EXIST, false, values);
	...
	public static boolean delete(int id) {
		...
		try {
			connection = connectionPool.checkOut();
			PreparedStatement pstmt = DAOUtil.prepareStatement(connection, SQL_DELETE_LOCATION_WITH_ID, false, values);
			pstmt.executeUpdate();
			idDeleted = true;
			pstmt.close();
		} catch (SQLException exp) {
